 ...
 */
add_action( 'wp_update_nav_menu_item', 'reino_custom_nav_update', 10, 3 );
function reino_custom_nav_update( $menu_id, $menu_item_db_id, $args ) {
 ...
 */
add_filter( 'wp_setup_nav_menu_item', 'reino_custom_nav_item' );
function reino_custom_nav_item( $menu_item ) {
	$menu_item->custom = get_post_meta( $menu_item->ID, 'menu-item-iva-megamenu', true );
	return $menu_item;
}

add_filter( 'wp_edit_nav_menu_walker', 'reino_custom_nav_edit_walker', 10, 2 );
function reino_custom_nav_edit_walker( $walker, $menu_id ) {
	return 'Reino_Walker_Nav_Menu_Edit_Custom';
}
 ...
 */
class Reino_Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
	/**
	...
	public function start_el( &$output, $object, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if ( ! empty( $object->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', 'reino' ), $object->title );
		} elseif ( isset( $object->post_status ) && 'draft' === $object->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__( '%s (Pending)', 'reino' ), $object->title );
		}

		$title = empty( $object->label ) ? $title : $object->label;

		?>
		<li id="menu-item-<?php echo esc_attr( $object_id ); ?>" class="<?php echo implode( ' ', esc_attr( $classes ) ); ?>">
				...
				<div class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>
							"><class="item-move-up"><abbr title="
							<?php
							esc_attr_e( 'Move up', 'reino' );
							?>
							">&#8593;</abbr></a>
							|
							<a href="
							...
							?>
							">
							<class="item-move-down"><abbr title="<?php esc_attr_e( 'Move down', 'reino' ); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr( $object_id ); ?>" title="<?php esc_attr_e( 'Edit Menu Item', 'reino' ); ?>" href="
							<?php
							echo ( esc_attr( $_GET['edit-menu-item'] ) && $object_id === $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', esc_attr( $object_id ), esc_attr( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $object_id ) ) );
						?>
						">
						<?php esc_html_e( 'Edit Menu Item', 'reino' ); ?></a>
					</span>
				</div>
			</div>
			...
			<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr( $object_id ); ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr( $object_id ); ?>">
						<?php esc_html_e( 'Navigation Label', 'reino' ); ?><br />
				...
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr( $object_id ); ?>">
						<?php esc_html_e( 'Title Attribute', 'reino' ); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr( $object_id ); ?>">
						...
						<?php esc_html_e( 'Open link in a new window/tab', 'reino' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr( $object_id ); ?>">
						<?php esc_html_e( 'CSS Classes (optional)', 'reino' ); ?><br />
				...
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr( $object_id ); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)', 'reino' ); ?><br />
				...
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr( $object_id ); ?>">
						<?php esc_html_e( 'Description', 'reino' ); ?><br />
						...
						<span class="description"><?php esc_html_e( 'The description will be displayed in the menu if the current theme supports it.', 'reino' ); ?></span>
					</label>
				</p>
				<?php
				if ( 0 === $depth ) :
				...
				<p class="field-custom description description-wide">
					<label for="edit-menu-item-iva-megamenu-<?php echo esc_attr( $object_id ); ?>">
						<?php esc_html_e( 'Mega Menu', 'reino' ); ?><br />
						<?php
						...
						?>
						<input type="checkbox" id="edit-menu-item-iva-megamenu-<?php echo esc_attr( $object_id ); ?>" class="widefat code edit-menu-item-custom" name="menu-item-iva-megamenu[<?php echo esc_attr( $object_id ); ?>]" <?php echo esc_attr( $value ); ?> />  <?php esc_html_e( 'Activate Mega Menu', 'reino' ); ?>
					</label>
				</p>
				<?php
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if ( 'custom' !== $object->type && false !== $original_title ) : ?>
						<p class="link-to-original">
							<!-- translators: %s: search term -->
							<?php printf( esc_html__( 'Original: %s', 'reino' ), '<a href="' . esc_url( $object->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					<a class="item-delete submitdelete deletion" id="delete-<?php echo esc_attr( $object_id ); ?>" href="
						<?php
						echo esc_attr(
							add_query_arg(
								array(
						...
						); ?>"><?php esc_html_e( 'Remove', 'reino' ); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $object_id ); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $object_id, 'cancel' => time() ), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
						?>#menu-item-settings-<?php echo esc_attr( $object_id ); ?>"><?php esc_html_e( 'Cancel', 'reino' ); ?></a>
				</div>
